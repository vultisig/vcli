# vcli - Vultisig Plugin Development CLI
...
```bash
cd verifier/cmd/vcli
CGO_ENABLED=1 go build -o vcli .
```
...
```bash
./vcli vault import --file /path/to/vault-share.vult --password "your-password"
```
...
```bash
./vcli plugin install vultisig-dca-0000 --password "your-password"
```
...
./vcli vault import --file <file.vult> --password <password>

# Export current vault to file
./vcli vault export [--output <file.json>]

# Show current vault information
./vcli vault info

# Set active vault (by public key prefix)
./vcli vault use <public-key-prefix>

# Generate a new vault with Fast Vault Server (2-of-2)
./vcli vault generate [--name <vault-name>] [--dry-run]

# Show vault addresses on chains
./vcli vault address [--chain <chain>]

# Show vault balances on chains
./vcli vault balance [--chain <chain>]

# Sign a message using TSS keysign
./vcli vault keysign --message <hex-hash> --password <password> [--derive <path>] [--eddsa]

# Reshare vault to add verifier and plugin
./vcli vault reshare --plugin <plugin-id> --password <password> [--verifier <url>]
```
...
# Show plugin details
./vcli plugin info <plugin-id>

# Install plugin (4-party reshare)
./vcli plugin install <plugin-id> --password <password>

# Uninstall plugin
./vcli plugin uninstall <plugin-id>

# Show plugin recipe specification
./vcli plugin spec <plugin-id>
```
...
# Delete a policy
./vcli policy delete <policy-id>

# Show policy transaction history
./vcli policy history <policy-id>
```
...
./vcli auth login [--vault <vault-id>] [--password <password>]
...
# Start services
./vcli services start --services <service1,service2,...>
./vcli services start --all

# Stop services
./vcli services stop [--all]

# View service logs
./vcli services logs [--service <name>] [--follow]
```
...
./vcli verify policy <policy-id>

# Check transaction history
./vcli verify transactions --policy <policy-id> [--limit <n>]
./vcli verify transactions --plugin <plugin-id> [--limit <n>]
```
...
Configuration is stored in `~/.vultisig/vcli.json` and is managed automatically by the CLI.
...
Next step: Add a policy with 'vcli policy add --plugin vultisig-dca-0000'
```
...
```
┌─────────────┐     ┌──────────────────┐     ┌─────────────────┐
│   vcli    │────▶│  Fast Vault API  │────▶│  Fast Vault DB  │
